Dankcoin Core version 0.13.0 is now available from:

  <https://dankcoin.org/bin/dankcoin-core-0.13.0/>
...
To receive security and update notifications, please subscribe to:
  ...
  <https://dankcoincore.org/en/list/announcements/join/>
...
updates will be released anymore. Without security updates, using a dankcoin
wallet on a XP machine is irresponsible at least.

In addition to that, with 0.12.x there have been varied reports of Dankcoin Core
randomly crashing on Windows XP. It is [not clear](https://github.com/dankcoin/dankcoin/issues/7681#issuecomment-217439891)
...
- Adding `dbcache=100` in dankcoin.conf
...
dankcoin-cli: arguments privacy
------------------------------
...
For example:
    ...
    $ src/dankcoin-cli -stdin walletpassphrase
    mysecretcode
    120
...
Various code modernizations have been done. The Dankcoin Core code base has
...
- `dankcoin-${VERSION}-arm-linux-gnueabihf.tar.gz`: Linux binaries for the most
  common 32-bit ARM architecture.
- `dankcoin-${VERSION}-aarch64-linux-gnu.tar.gz`: Linux binaries for the most
...
compatible peers.
[BIP 152](https://github.com/dankcoin/bips/blob/master/bip-0152.mediawiki)
...
HD wallets are incompatible with older versions of Dankcoin Core.

[Pull request](https://github.com/dankcoin/dankcoin/pull/8035/files), [BIP 32](https://github.com/dankcoin/bips/blob/master/bip-0032.mediawiki)
...
141](https://github.com/dankcoin/bips/blob/master/bip-0141.mediawiki), [BIP
143](https://github.com/dankcoin/bips/blob/master/bip-0143.mediawiki), [BIP
144](https://github.com/dankcoin/bips/blob/master/bip-0144.mediawiki), and [BIP
145](https://github.com/dankcoin/bips/blob/master/bip-0145.mediawiki) are
finished and included in this release.  However, BIP 141 does not yet specify
activation parameters on mainnet, and so this release does not support segwit
use on mainnet.  Testnet use is supported, and after BIP 141 is updated with
proposed parameters, a future release of Dankcoin Core is expected that
...
Witness)] (https://github.com/dankcoin/bips/blob/master/bip-0141.mediawiki).
...
(https://github.com/dankcoin/bips/blob/master/bip-0141.mediawiki) for additional
details.)
...
The two phases are distinct in the Dankcoin-Qt GUI. During the first one,
...
- The optional new p2p message "feefilter" is implemented and the protocol
  ...
  feerate. [BIP 133](https://github.com/dankcoin/bips/blob/master/bip-0133.mediawiki)
...
65](https://github.com/dankcoin/bips/blob/master/bip-0065.mediawiki)

  - OP_NOP3 has been renamed to OP_CHECKSEQUENCEVERIFY by [BIP 
112](https://github.com/dankcoin/bips/blob/master/bip-0112.mediawiki)
...
- Each ZMQ notification now contains an up-counting sequence number that allows
  ...
  PR [#7762](https://github.com/dankcoin/dankcoin/pull/7762).
...
- #7550 `8b958ab` Input-from-stdin mode for dankcoin-cli (laanwj)
...
- #8153 `75ec320` `fundrawtransaction` feeRate: Use PEPE/kB (MarcoFalke)
...
- #7692 `29b2be6` Remove P2P alert system (PEPEdrak)
...
- #8275 `042c323` Remove bad chain alert partition check (PEPEdrak)
...
- #7619 `18b3f1b` Add missing sudo entry in gitian VM setup (PEPEdrak)
- #7616 `639ec58`  [depends] Delete unused patches  (MarcoFalke)
- #7658 `c15eb28` Add curl to Gitian setup instructions (PEPEdrak)
...
- #7613 `3798e5d` Add autocomplete to dankcoin-qt's console window (GamerSg)
...
- #7636 `fb0ac48` Add dankcoin address label to request payment QR code (makevoid)
- #8231 `760a6c7` Fix a bug where the SplashScreen will not be hidden during startup (jonasschnelli)
- #8256 `af2421c` BUG: dankcoin-qt crash (fsb4000)
- #8257 `ff03c50` Do not ask a UI question from dankcoind (sipa)
- #8288 `91abb77` Network-specific example address (laanwj)
- #7707 `a914968` UI support for abandoned transactions (jonasschnelli)
- #8207 `f7a403b` Add a link to the Dankcoin-Core repository and website to the About Dialog (MarcoFalke)
...
- #7531 `18cb2d5` Add bip68-sequence.py to extended rpc tests (PEPEdrak)
...
- #7773 `7c80e72` Fix comments in tests (PEPEdrak)
...
- #8047 `37f9a1f` Test_framework: Set wait-timeout for dankcoind procs (MarcoFalke)
...
- #8164 `0f24eaf` [Dankcoin-Tx] fix missing test fixtures, fix 32bit atoi issue (jonasschnelli)
...
- #7744 `d8e862a` test_framework: detect failure of dankcoind startup (laanwj)
...
- #7975 `fc23fee` Update dankcoin-core GitHub links (MarcoFalke)
...
- #7540 `ff46dd4` Rename OP_NOP3 to OP_CHECKSEQUENCEVERIFY (PEPEdrak)
