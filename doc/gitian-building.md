...
*Setup instructions for a Gitian build of Dankcoin Core using a Debian VM or physical system.*
...
- [Building Dankcoin Core](#building-dankcoin-core)
...
- [LXC](https://linuxcontainers.org/), see also [Gitian host docker container](https://github.com/gdm85/tenku/tree/master/docker/gitian-dankcoin-host/README.md).
...
Clone the git repositories for dankcoin and Gitian.
...
in the dankcoin repository under 'Fetch and build inputs' to install sources which require
manual intervention. Also optionally follow the next step: 'Seed the Gitian sources cache
and offline git repositories' which will fetch the remaining files required for building
offline.

Building Dankcoin Core
----------------

To build Dankcoin Core (for Linux, OS X and Windows) just follow the steps under 'perform
Gitian builds' in [doc/release-process.md](release-process.md#perform-gitian-builds) in the dankcoin repository.
...
For example:
```bash
URL=https://github.com/laanwj/dankcoin.git
...
and the dankcoin git repository with the desired tag must both be available locally, and then gbuild must be
...
LXC_ARCH=amd64 LXC_SUITE=trusty on-target -u root \
  ...
  $( sed -ne '/^packages:/,/[^-] .*/ {/^- .*/{s/"//g;s/- //;p}}' ../dankcoin/contrib/gitian-descriptors/*|sort|uniq )
...
PEPEPATH=/some/root/path/dankcoin
SIGPATH=/some/root/path/dankcoin-detached-sigs

./bin/gbuild --url dankcoin=${PEPEPATH},signature=${SIGPATH} ../dankcoin/contrib/gitian-descriptors/gitian-win-signer.yml
```
...
```bash
    gpg --detach-sign ${VERSION}-linux/${SIGNER}/dankcoin-linux-build.assert
    gpg --detach-sign ${VERSION}-win/${SIGNER}/dankcoin-win-build.assert
    gpg --detach-sign ${VERSION}-osx-unsigned/${SIGNER}/dankcoin-osx-build.assert
```
...
[dankcoin-core/gitian.sigs](https://github.com/dankcoin-core/gitian.sigs/) repository, or if that's not possible create a pull
