Dankcoin Core version 0.11.0 is now available from:

  <https://dankcoin.org/bin/dankcoin-core-0.11.0/>
...
defaults to 0.00001. This will cause transactions with fewer PEPE/kB fee to be
...
For example, add the following to `dankcoin.conf`:
...
related to the blockchain in the dankcoin system: the raw blocks as received over 
the network (blk???.dat), the undo data (rev???.dat), the block index and the 
UTXO set (both LevelDB databases). The databases are built from the raw data.

Block pruning allows Dankcoin Core to delete the raw block and undo data once 
...
block index and UTXO databases. The minimum was chosen so that Dankcoin Core will 
...
For an example script see [dankcoin-submittx](https://github.com/laanwj/dankcoin-submittx).
...
- #5334 `cf87536` libdankcoinconsensus: Add pkg-config support
...
- #6269 `95aca44` gitian: Use the new dankcoin-detached-sigs git repo for OSX signatures
...
- #5729 `9c4a5a5` Allow unit changes for read-only DankcoinAmountField
- #5753 `0f44672` Add dankcoin logo to about screen
...
- #5809 `46bfbe7` Add dankcoin-cli man page
...
- #5849 `41259ca` contrib/init/dankcoind.openrc: Compatibility with previous OpenRC init script variables
...
- #6116 `30dc3c1` [OSX] rename Dankcoin-Qt.app to Dankcoin-Core.app
- #6168 `b3024f0` contrib/linearize: Support linearization of testnet blocks
- #6098 `7708fcd` Update Windows resource files (and add one for dankcoin-tx)
