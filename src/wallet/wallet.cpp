...
bool CWallet::LoadCScript(const CScript& redeemScript)
{
    ...
    {
        std::string strAddr = CDankcoinAddress(CScriptID(redeemScript)).ToString();
        ...
        LOCK2(cs_main, cs_wallet);
        {
            ...
            while (true)
            {
                ...
                if (nChange > 0)
                {
                    ...
                    // change transaction isn't always pay-to-dankcoin-address
...
bool CWallet::SetAddressBook(const CTxDestination& address, const string& strName, const string& strPurpose)
{
    ...
    if (!fFileBacked)
        return false;
    if (!strPurpose.empty() && !CWalletDB(strWalletFile).WritePurpose(CDankcoinAddress(address).ToString(), strPurpose))
        return false;
    return CWalletDB(strWalletFile).WriteName(CDankcoinAddress(address).ToString(), strName);
}
...
{
    {
        ...
        if(fFileBacked)
        {
            ...
            std::string strAddress = CDankcoinAddress(address).ToString();
    ...
    if (!fFileBacked)
        return false;
    CWalletDB(strWalletFile).ErasePurpose(CDankcoinAddress(address).ToString());
    return CWalletDB(strWalletFile).EraseName(CDankcoinAddress(address).ToString());
}
...
bool CWallet::AddDestData(const CTxDestination &dest, const std::string &key, const std::string &value)
{
    ...
    if (!fFileBacked)
        return true;
    return CWalletDB(strWalletFile).WriteDestData(CDankcoinAddress(dest).ToString(), key, value);
}
...
bool CWallet::EraseDestData(const CTxDestination &dest, const std::string &key)
{
    ...
    if (!fFileBacked)
        return true;
    return CWalletDB(strWalletFile).EraseDestData(CDankcoinAddress(dest).ToString(), key);
}
