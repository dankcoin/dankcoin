...
void ScriptPubKeyToJSON(const CScript& scriptPubKey, UniValue& out, bool fIncludeHex)
{
    ...
    BOOST_FOREACH(const CTxDestination& addr, addresses)
        a.push_back(CDankcoinAddress(addr).ToString());
        ...
        if (tx.IsCoinBase())
            ...
            if (GetSpentIndex(spentKey, spentInfo)) {
                ...
                if (spentInfo.addressType == 1) {
                    in.push_back(Pair("address", CDankcoinAddress(CKeyID(spentInfo.addressHash)).ToString()));
                } else if (spentInfo.addressType == 2)  {
                    in.push_back(Pair("address", CDankcoinAddress(CScriptID(spentInfo.addressHash)).ToString()));
                }
            }
...
UniValue createrawtransaction(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() < 2 || params.size() > 3)
        throw runtime_error(
            ...
            "    {\n"
            "      \"address\": x.xxx   (numeric or string, required) The key is the dankcoin address, the numeric value (can be string) is the " + CURRENCY_UNIT + " amount\n"
...
UniValue decoderawtransaction(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() != 1)
        throw runtime_error(
            ...
            "         \"type\" : \"pubkeyhash\",  (string) The type, eg 'pubkeyhash'\n"
            "         \"addresses\" : [           (json array of string)\n"
            "           \"12tvKAXCxZjSmdNbao16dKXC8tRWfcF5oc\"   (string) dankcoin address\n"
            "           ,...\n"
            "         ]\n"
            "       }\n"
            "     }\n"
...
UniValue decodescript(const UniValue& params, bool fHelp)
{
    ...
    r.push_back(Pair("p2sh", CDankcoinAddress(CScriptID(script)).ToString()));
    return r;
}
