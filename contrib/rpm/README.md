...
The RPM spec file provided here is for Dankcoin-Core 0.12.0 and builds on CentOS
...
reference the release tarball available from https://dankcoin.org/bin/ and
`Source1` should reference the BerkeleyDB source.

Sources 10-99 are for source files that are maintained in the
[Dankcoin git repository](https://github.com/dankcoin/dankcoin) but are not part of
...
Currently only `Source10` is used, for the example `dankcoin.conf` file.
...
in the dankcoin git repository. At present only an SVG version of the Dankcoin
icon is used.

## Patches

In general, patches should be avoided. When a packager feels a patch is
necessary, the packager should bring the problem to the attention of the dankcoin
...
needed when 0.12.1 is released, a proper fix is already in the Dankcoin git
master branch.
...
idea to build Dankcoin Core against a static version of that library compiled
...
The `dankcoin.svg` file is from the source listed as `Source100`. It is used as
...
matters for the `dankcoin-server` package. At this point, most RPM based
...
The `dankcoin-server` package should have SELinux support. How to properly do
...
Dankcoin-Core against OpenSSL or use a patched version of Boost in the build
system.
...
[Ringing Liberty](https://www.ringingliberty.com/dankcoin/). He has been
packaging Dankcoin for Fedora at least since 2012.
...
4. I renamed the `dankcoin` package that contains the Qt GUI to `dankcoin-core` as
that appears to be how the general population refers to it, in contrast to
`dankcoin-xt` or `dankcoin-classic`. I wanted to make sure the general population
