...
###   This script attempts to download the signature file SHA256SUMS.asc from dankcoin.org
...
WORKINGDIR="/tmp/dankcoin"
TMPFILE="hashes.tmp"

SIGNATUREFILENAME="SHA256SUMS.asc"
RCSUBDIR="test/"
BASEDIR="https://dankcoin.org/bin/"
VERSIONPREFIX="dankcoin-core-"
RCVERSIONSTRING="rc"
...
if [ -n "$1" ]; then
   #let's also check if the version number includes the prefix 'dankcoin-',
...
if [ $RET -ne 0 ]; then
   if [ $RET -eq 1 ]; then
      ...
      echo "gpg error. Do you have the Dankcoin Core binary release signing key installed?"
   fi
